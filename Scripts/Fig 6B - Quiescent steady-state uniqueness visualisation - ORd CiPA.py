...
def run_sim(concs):
    ...
    try:
        run_sim = s.run(4000001, log=['intracellular_ions.ki', 'intracellular_ions.kss', 'intracellular_ions.nai', 'intracellular_ions.nass', 'membrane.V', 'intracellular_ions.cai', 'intracellular_ions.cass', 'intracellular_ions.cajsr', 'intracellular_ions.cansr'], log_times = [0, 4000000])
    except:
        print('This simulation failed.')
    return run_sim


#G0 = G0_calc(Nai = default[1], Nass = default[2], Ki = default[3], Kss = default[4], Cai = default[5], Cass = default[6], Cansr = default[7]], Cajsr = default[8], V = -85.3)
...
G0s = np.linspace(-20, 15, nb_planes)
